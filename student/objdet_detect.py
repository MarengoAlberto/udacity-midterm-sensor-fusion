...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        # perform inference
        outputs = model(input_bev_maps.float())
        ...
        if 'darknet' in configs.arch:
            ...
            # Where the 10 are the following values: [score,x,y,z,dim(h,w,l),direction(sinθ,cosθ),class]
            detections = decode(outputs['hm_cen'], outputs['cen_offset'], outputs['direction'], outputs['z_coor'],
                                outputs['dim'], K=40)  # K=configs.k
            detections = detections.cpu().numpy().astype(np.float32)
            # Get detections in format [{0: array(... shape [N0, 8] ...), 1: array(... shape [N1, 8] ...), ...}, .... {}]
            # where 0, 1, ... n_classes
            # where N0, N1, ... are the number of detections for each class
            # where the 8 are the following values: [score,x,y,z,h,w,l,yaw]
            detections = post_processing(detections, configs)
            # We select the first element of the batch because we only have one image
            # We select only car detections which are class 1
