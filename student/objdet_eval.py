...
def calculate_iou(gt_bbox, pred_bbox):
    ...
    if not (x2 < x3 or x4 < x1 or y2 < y3 or y4 < y1):
        x_inter_tl = max(x1, x3)
        y_inter_tl = max(y1, y3)
        x_inter_br = min(x2, x4)
        y_inter_br = min(y2, y4)

        # Width and height of intersection
        inter_width = max(0, x_inter_br - x_inter_tl)
        inter_height = max(0, y_inter_br - y_inter_tl)

        # Area of intersection
        area_intersection = inter_width * inter_height

        # Areas of the individual rectangles
        area1 = (x2 - x1) * (y2 - y1)
        area2 = (x4 - x3) * (y4 - y3)
        ...
        # IoU calculation
        if area_union == 0:
            iou = 0  # To avoid division by zero
...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box = label.box
            box_true = tools.compute_box_corners(box.center_x, box.center_y, box.width, box.length, box.heading)
            # box_true = (box.center_x - box.width/2, box.center_y - box.length/2, box.center_x + box.width/2, box.center_y + box.length/2)
            ## step 2 : loop over all detected objects
            for detection in detections:
                ## step 3 : extract the four corners of the current detection
                _id, x, y, z, _h, w, l, yaw = detection
                box_det = tools.compute_box_corners(x, y, w, l, yaw)
                # box_det = (x - w/2, y - l/2, x + w/2, y + l/2)
                ...
                # iou = calculate_iou(box_true, box_det)
                poly_true = Polygon(box_true)
                poly_det = Polygon(box_det)
                intersection = poly_true.intersection(poly_det).area
                union = poly_true.union(poly_det).area
